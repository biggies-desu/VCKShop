...
function Allcarmodels() {
    ...
    const { selectedBrand: initialSelectedBrand } = location.state || {};
    ...
    useEffect(() => {
        if (initialSelectedBrand) setSelectedBrand(initialSelectedBrand);
    }, [initialSelectedBrand]);

    useEffect(() => {
        axios.get(`${import.meta.env.VITE_API_URL}/model-list`)
            .then((response) => {
                setModelList(response.data);
                console.log("ModelList",response.data)
            })
            .catch((error) => {
    ...
    if (modelId) {
        fetchAllData(modelId);
            console.log("Fetching sparepart for modelId:", modelId);
        }
    }, [modelId]);
    ...
    function sortByCategory(categoryId, keyword) {
        axios.get(`${import.meta.env.VITE_API_URL}/sparepartcategory?modelId=${modelId}&category=${categoryId}`)
        .then((res) => {
    ...
    const search = (event) => {
        ...
        axios.post(`${import.meta.env.VITE_API_URL}/searchquery`, { search_query, modelId })
        .then((res) => {
    ...
    const groupedBrands = modelList.reduce((acc, m) => {
        if (!acc[m.brand])
            acc[m.brand] = [];
            acc[m.brand].push(m);
        return acc;
    ...
    if (!Array.isArray(groupedBrands[selectedBrand])) {
        return (
          <>
            <Navbar />
    ...
    return (
        <>
            <Navbar />
            <div className="bg-orange-100 min-h-screen py-10 px-6">
                {selectedModel === null ? (
                                ...
                                <div className={`${isOpen ? 'block' : 'hidden'} lg:hidden absolute inset-x-0 h-full w-full max-w-[20rem] z-30 px-6 py-4 transition-all duration-300 ease-in-out lg:mt-0 lg:p-0 lg:top-0 lg:relative lg:w-auto lg:translate-x-0 lg:items-center`}>
                                    <div className="fixed z-[9999] pointer-events-auto bg-white box-border w-full shadow-2xl shadow-blue-gray-900/10 top-0 left-0"
                                        ...
                                        <Card className="bg-white opacity-80 h-[calc(100vh-2rem)] w-full max-w-[20rem] p-4 shadow-xl shadow-blue-gray-900/5 overflow-y-auto max-h-screen scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
                                            ...
                                            <List>
                                                <ListItem className="hover:text-yellow-800 hover:scale-110" onClick={() => fetchAllData(modelId)}>สินค้าทั้งหมด 
                                                    <ListItemSuffix>
                                ...
                                <div className="lg:flex hidden">
                                    <Card className="bg-white opacity-80 h-fit w-full max-w-[20rem] p-4 shadow-xl shadow-blue-gray-900/5">
                                        ...
                                        <div className="relative p-2">
                                            ...
                                            <label className="absolute left-4 -top-4 text-gray-400 opacity-80 text-base transition-all duration-100 peer-placeholder-shown:top-4 peer-placeholder-shown:text-base peer-focus:top-[-15px] peer-focus:text-sm peer-focus:text-blue-500">Search...</label>
                                            <svg id="clear" onClick={clear} className="absolute right-12 top-1/2 transform -translate-y-1/2 size-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" >
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M5.29289 5.29289C5.68342 4.90237 6.31658 4.90237 6.70711 5.29289L12 10.5858L17.2929 5.29289C17.6834 4.90237 18.3166 4.90237 18.7071 5.29289C19.0976 5.68342 19.0976 6.31658 18.7071 6.70711L13.4142 12L18.7071 17.2929C19.0976 17.6834 19.0976 18.3166 18.7071 18.7071C18.3166 19.0976 17.6834 19.0976 17.2929 18.7071L12 13.4142L6.70711 18.7071C6.31658 19.0976 5.68342 19.0976 5.29289 18.7071C4.90237 18.3166 4.90237 17.6834 5.29289 17.2929L10.5858 12L5.29289 6.70711C4.90237 6.31658 4.90237 5.68342 5.29289 5.29289Z" fill="#0F1729"/>
                                            </svg>
