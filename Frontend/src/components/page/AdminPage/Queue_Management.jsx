...
function Queue_Management()
{
    ...
    const handleDateChange = (selectedDate) => {
        ...
        if (selectedDate) {
            axios.post(`${import.meta.env.VITE_API_URL}/checkQueue`, {
                date: selectedDate
            })
            .then((res) => {
    ...
    const handleTimeChange = (selectedDate, selectedTime) => {
        setTime(selectedTime);
        setEditQueue(prev => ({ ...prev, Booking_Time: selectedTime }));
    
        if (selectedDate && selectedTime) {
            axios.post(`${import.meta.env.VITE_API_URL}/checkQueue`, { date: selectedDate, time: selectedTime })
                .then((res) => {
                    const { queueCount, isFull } = res.data;
                    if (isFull || queueCount >= 3) {
                        setDisabledTimes(prev => [...new Set([...prev, selectedTime])]);
                    } else {
                        setDisabledTimes(prev => prev.filter(time => time !== selectedTime));
                    }
                })
    ...
    function confirmDelete(index)
    {
        ...
        axios.delete(`${import.meta.env.VITE_API_URL}/deletequeue/${deleteId}`)
        .then((res) => {
            console.log(res)
            fetchdata()
            closeModal();
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md md:max-w-lg lg:max-w-xl mx-4">
                    ...
                    <form onSubmit={e => { e.preventDefault(); updateQueue(); }}>
                        ...
                        <div className="mb-4">
                            ...
                            <select id="time" className="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={editQueue.Booking_Time} onChange={(e) => handleTimeChange(editQueue.Booking_Date, e.target.value)}>
