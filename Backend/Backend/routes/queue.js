...
router.get('/allqueue', function (req, res) {
  const sqlcommand = `
    ...
    GROUP_CONCAT(bt.Technician_ID) AS Technician_IDs,
...
router.post('/updatequeue_status', async function (req, res) {
  ...
  db.query(sql, [status, booking_id], function (err, result) {
    ...
    if (status === 3) {
      ...
      db.query(sparepartSql, [booking_id], async (err2, result2) => {
        if (err2) return res.status(500).json({ error: err2 });

        try {
          // After updating stock, check if any spare parts have a quantity of 0 and should be notified
          const checkZeroSql = `SELECT s.SparePart_ID, s.SparePart_Name, s.SparePart_Notify
                                FROM Sparepart s
                                WHERE s.SparePart_Amount = 0 AND s.SparePart_Notify = 1`;

          db.query(checkZeroSql, async (err3, zeroParts) => {
            if (err3) {
              console.error('Error checking zero spareparts:', err3);
              return res.json({ message: 'Updated status and reduced spare parts, but couldn\'t check zero parts.' });
            }

            if (zeroParts.length > 0) {
              console.log('There are spare parts with 0 quantity that need to be notified, sending LINE Notify...');

              // Here you can send specific info about the spare parts that are 0
              const notifyMessage = zeroParts.map(part => `${part.SparePart_Name} is now out of stock!`).join('\n');

              // Send LINE notification with the names of the spare parts
              await sendtoline(notifyMessage);  // Pass the custom message to sendtoline
            }

            return res.json({ message: 'Updated status and reduced spare parts successfully.' });
          });

        } catch (e) {
          console.error('Error during zero-check or sending LINE:', e);
          return res.json({ message: 'Updated status and reduced spare parts, but couldn\'t send LINE notification.' });
        }
      });

    } else {
      return res.json({ message: 'Updated status successfully.' });
    }
  });
});
